...
const FloatingOrb: React.FC<OrbProps> = ({ size, color, top, left, delay, duration }) => (
  <div
    aria-hidden="true"
    style={{
...
const Home: React.FC = () => {
  ...
  const featuredCerts = useMemo(
    () => allCerts.filter((c) => c.is_featured).slice(0, 6),
    [allCerts]
  );
  const displayedCerts = featuredCerts.length > 0 ? featuredCerts : allCerts.slice(0, 6);
  ...
  /* Cert stat helpers */
  const certValidCount = useMemo(
    () => allCerts.filter((c) => !certIsExpired(c.expiry_date)).length,
    [allCerts]
  );
  const certIssuerCount = useMemo(
    () =>
      new Set(
        allCerts
  ...
  function cxLabel(score: number): string {
    if (score <= 3) return "Beginner";
    if (score <= 6) return "Intermediate";
    return "Advanced";
  }
  function cxClass(score: number): string {
    if (score <= 3) return "proj-cx-1";
    if (score <= 6) return "proj-cx-2";
    return "proj-cx-3";
  }
  ...
  return (
    <>
      ...
      <header className={`top-navbar${scrolled ? " scrolled" : ""}`}>
        <div className="nav-container">
          ...
          <nav className="desktop-nav">
            ...
            <a
              href="https://instagram.com/chhava.ai"
              target="_blank"
              rel="noreferrer"
              className="nav-link"
            >
            ...
            {/* The Link to the Admin route is correctly restored here */}
        ...
        <div className="marquee-viewport">
          <div className="marquee-track">
            ...
            {[...displayProjects, ...displayProjects].map((p, i) => (
              <div key={`marquee-${p.project_id}-${i}`} className="marquee-item pop-in" style={{ animationDelay: `${(i % displayProjects.length) * 0.15}s` }}>
      ...
      <style>{`
        ...
        @keyframes floatGlow   { 0%{transform:translate(0,0) scale(1)} 100%{transform:translate(30px,40px) scale(1.1)} }
        @keyframes orbFloat    { 0%{transform:translateY(0) scale(1) rotate(0deg)} 100%{transform:translateY(-30px) scale(1.08) rotate(5deg)} }
        ...
        * { box-sizing:border-box; }
        ...
        .subtitle { color:var(--li-text-muted); font-size:17px; line-height:1.65; max-width:580px; }

        /* Aurora */
        .aurora-container { position:fixed; top:0; left:0; width:100%; height:100vh; overflow:hidden; z-index:0; pointer-events:none; }
        .glow { position:absolute; border-radius:50%; filter:blur(140px); opacity:.4; mix-blend-mode:multiply; animation:floatGlow 10s ease-in-out infinite alternate; }
        .glow-cyan   { width:600px;height:600px;background:var(--li-cyan);  top:-100px;left:10%;  animation-delay:0s; }
        .glow-purple { width:500px;height:500px;background:var(--li-purple);top:20%;  left:40%;  opacity:.3; animation-delay:-3s; }
        .glow-pink   { width:550px;height:550px;background:var(--li-pink);  top:10%;  right:10%; opacity:.25;animation-delay:-6s; }
        ...
        .nav-link { text-decoration:none; color:var(--li-text-muted); font-weight:600; font-size:14px; transition:all .2s ease; display:flex; align-items:center; gap:6px; position:relative; }
        .nav-link::after { content:""; position:absolute; bottom:-3px; left:0; width:0; height:2px; background:linear-gradient(90deg,var(--li-purple),var(--li-cyan)); transition:width .25s ease; border-radius:1px; }
        ...
        .nav-badge  { background:rgba(55,215,250,.15); color:#0369a1; font-family:'IBM Plex Mono',monospace; font-size:10px; font-weight:700; padding:2px 6px; border-radius:4px; text-transform:uppercase; }

        /* Buttons */
        .primary-btn,.outline-btn,.nav-btn { padding:12px 24px; border-radius:6px; font-weight:700; font-size:14px; cursor:pointer; transition:all .22s ease; text-decoration:none; display:inline-flex; align-items:center; justify-content:center; gap:6px; }
        .primary-btn { background:var(--li-text-main); color:#fff; border:none; box-shadow:0 4px 14px rgba(0,0,0,.10); }
        .primary-btn:hover { background:var(--li-purple); transform:translateY(-2px); box-shadow:0 8px 22px rgba(62,24,249,.25); }
        .primary-btn:active { transform:scale(.97); }
        .outline-btn { background:white; color:var(--li-text-main); border:1.5px solid var(--li-border); }
        .outline-btn:hover { border-color:var(--li-text-main); transform:translateY(-1px); }

        /* Section */
        .section { padding:120px 5%; display:flex; flex-direction:column; align-items:center; position:relative; z-index:10; }
        .bg-light-grey { background:#F7F8FC; border-top:1px solid var(--li-border); border-bottom:1px solid var(--li-border); }
        ...
        .social-chip { display:inline-flex; align-items:center; gap:6px; padding:7px 14px; border-radius:999px; border:1px solid var(--li-border); background:white; font-size:13px; font-weight:600; color:var(--li-text-muted); text-decoration:none; transition:all .2s ease; }
        .social-chip:hover { border-color:var(--li-purple); color:var(--li-purple); transform:translateY(-2px); box-shadow:0 4px 12px rgba(62,24,249,.12); }

        /* Hero */
        .hero { min-height:100vh; display:flex; align-items:center; justify-content:center; padding:120px 5% 80px; position:relative; z-index:10; }
        .hero-content { display:flex; align-items:center; justify-content:space-between; max-width:1200px; width:100%; gap:60px; }
        .hero-text { flex:1; display:flex; flex-direction:column; align-items:flex-start; max-width:560px; }
        .hero h1 { font-size:clamp(42px,5.5vw,72px); line-height:1.06; margin-bottom:20px; font-weight:900; }
        .hero-title-line { font-size:clamp(20px,2.8vw,32px); font-weight:700; color:var(--li-text-muted); letter-spacing:-.02em; -webkit-text-fill-color:var(--li-text-muted); background:none; }
        .tagline-badge { display:inline-flex; align-items:center; gap:8px; padding:6px 16px; background:white; border:1px solid var(--li-border); border-radius:20px; font-size:13px; font-weight:600; color:var(--li-text-muted); margin-bottom:24px; box-shadow:0 2px 8px rgba(0,0,0,.03); }
        .dot { width:8px; height:8px; background:#10B981; border-radius:50%; animation:pulse 2s infinite; flex-shrink:0; }
        ...
        .hero-stat-num { font-size:32px; font-weight:900; letter-spacing:-.04em; line-height:1; }
        .hero-stat-lbl { font-family:'IBM Plex Mono',monospace; font-size:11px; font-weight:700; color:var(--li-text-muted); text-transform:uppercase; letter-spacing:.05em; }

        .hero-visual  { flex:1; display:flex; justify-content:center; align-items:center; position:relative; min-height:480px; }
        .hero-3d-scene{ position:relative; width:380px; height:440px; display:flex; align-items:center; justify-content:center; }

        .profile-3d-card { position:relative; z-index:2; background:white; border-radius:24px; border:1px solid var(--li-border); box-shadow:0 32px 80px rgba(62,24,249,.12),0 8px 24px rgba(0,0,0,.06); overflow:hidden; width:280px; transform-style:preserve-3d; }
        .profile-card-glow { position:absolute; inset:0; background:linear-gradient(135deg,rgba(62,24,249,.06),rgba(55,215,250,.06)); pointer-events:none; }
        .profile-card-photo { width:100%; height:220px; object-fit:cover; display:block; }
        .profile-card-placeholder { width:100%; height:220px; background:linear-gradient(135deg,rgba(62,24,249,.10),rgba(55,215,250,.10)); display:flex; align-items:center; justify-content:center; font-size:80px; font-weight:900; background-size:200% 200%; animation:gradientShift 4s ease infinite; color:var(--li-purple); }
        .profile-card-info  { padding:18px 20px; }
        .profile-card-name  { font-size:18px; font-weight:900; margin:0 0 4px; letter-spacing:-.02em; }
        ...
        .float-badge { position:absolute; background:white; border:1px solid var(--li-border); border-radius:999px; font-size:12px; font-weight:800; padding:8px 14px; white-space:nowrap; box-shadow:0 8px 24px rgba(0,0,0,.08); z-index:3; font-family:'IBM Plex Mono',monospace; animation:badgeFloat 4s ease-in-out infinite; }
        ...
        /* About */
        .about-grid { display:grid; grid-template-columns:1fr 1.6fr; gap:60px; max-width:1100px; width:100%; align-items:start; }
        .about-photo-wrap { position:relative; border-radius:20px; overflow:hidden; box-shadow:0 24px 60px rgba(62,24,249,.12); }
        .about-photo { width:100%; display:block; border-radius:20px; }
        .about-photo-badge { position:absolute; bottom:20px; right:20px; background:white; border-radius:12px; padding:12px 16px; text-align:center; box-shadow:0 8px 24px rgba(0,0,0,.12); font-weight:800; font-size:12px; color:var(--li-text-muted); }
        .about-photo-num { display:block; font-size:28px; font-weight:900; background:linear-gradient(90deg,var(--li-purple),var(--li-cyan)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; line-height:1.1; }
        ...
        .about-skills-summary { font-size:14px; color:var(--li-text-muted); line-height:1.65; margin:0 0 20px; padding:14px 16px; border-radius:8px; background:rgba(62,24,249,.04); border:1px solid rgba(62,24,249,.12); }
        .about-socials { display:flex; flex-wrap:wrap; gap:10px; }
        .social-pill { padding:8px 16px; border-radius:6px; border:1px solid var(--li-border); background:white; font-size:13px; font-weight:700; color:var(--li-text-muted); text-decoration:none; transition:all .18s ease; font-family:'IBM Plex Mono',monospace; }
        ...
        .skill-cat-title { font-family:'IBM Plex Mono',monospace; font-size:11px; font-weight:800; color:var(--li-purple); text-transform:uppercase; letter-spacing:.10em; margin:0 0 16px; padding-bottom:8px; border-bottom:1px solid rgba(62,24,249,.15); }
        .skills-grid { display:flex; flex-wrap:wrap; gap:12px; }
        .skill-badge { font-family:'IBM Plex Mono',monospace; background:white; color:var(--li-text-main); padding:14px 22px; border-radius:6px; font-weight:600; font-size:14px; border:1px solid var(--li-border); box-shadow:0 2px 8px rgba(0,0,0,.02); transition:all .2s ease; }
        .skill-badge:hover { transform:translateY(-3px); border-color:var(--li-cyan); color:var(--li-purple); box-shadow:0 8px 20px rgba(55,215,250,.12); }
        .skill-badge-3d { cursor:default; }
        .skill-badge-inner { display:flex; align-items:center; gap:10px; background:white; border-radius:10px; border:1px solid; padding:10px 16px; min-width:120px; box-shadow:0 4px 12px rgba(0,0,0,.04); position:relative; overflow:hidden; }
        .skill-badge-inner::after { content:""; position:absolute; top:-40%; left:-40%; width:50%; height:180%; background:linear-gradient(90deg,transparent,rgba(255,255,255,.5),transparent); transform:skewX(-15deg); animation:shimmer 3s ease-in-out infinite; }
        .skill-icon-img    { width:24px; height:24px; object-fit:contain; flex-shrink:0; }
        .skill-icon-letter { width:24px; height:24px; flex-shrink:0; font-size:16px; font-weight:900; display:flex; align-items:center; justify-content:center; }
        ...
        .proj-link  { display:inline-flex; align-items:center; gap:5px; padding:6px 14px; border-radius:6px; font-size:12px; font-weight:700; text-decoration:none; transition:all .18s ease; border:1px solid; }
        ...
        .cert-stat {
          ...
          animation:certPopin .45s cubic-bezier(.34,1.56,.64,1) both;
        ...
        .cert-card--expired:hover { opacity:.85; filter:saturate(.75); }
        ...
        .cert-topbar {
          ...
          animation:certBarPulse 3s ease-in-out infinite;
        }
        .cert-topbar::after {
        ...
        /* Thumbnail */
        .cert-thumb {
          height:140px; overflow:hidden; flex-shrink:0; position:relative;
          background:linear-gradient(135deg,rgba(62,24,249,.06),rgba(55,215,250,.06));
        }
        .cert-thumb__img { width:100%; height:100%; object-fit:cover; display:block; transition:transform .4s ease; }
        ...
        .cert-btn { display:inline-flex; align-items:center; gap:4px; padding:6px 13px; border-radius:6px; font-size:11px; font-weight:800; text-decoration:none; font-family:'IBM Plex Mono',monospace; transition:all .18s ease; border:1px solid; }
        ...
        /* Contact */
        .contact-card { max-width:900px; width:100%; cursor:default; }
        .contact-card-inner { background:white; border-radius:20px; border:1px solid var(--li-border); box-shadow:0 24px 60px rgba(62,24,249,.08); padding:48px; display:flex; gap:48px; flex-wrap:wrap; position:relative; overflow:hidden; }
        .contact-card-inner::before { content:""; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,var(--li-purple),var(--li-cyan),var(--li-pink)); background-size:200% 100%; animation:gradientShift 3s ease infinite; }
        .contact-left  { flex:1; min-width:220px; }
        .contact-right { display:flex; flex-direction:column; gap:12px; align-items:flex-start; }
        .contact-left h3 { font-size:22px; font-weight:900; margin:0 0 12px; letter-spacing:-.02em; }
        .contact-left p  { color:var(--li-text-muted); font-size:15px; line-height:1.6; margin:0 0 24px; }
        .contact-links { display:flex; flex-direction:column; gap:12px; }
        .contact-link { display:flex; align-items:center; gap:10px; font-size:14px; font-weight:600; color:var(--li-text-muted); text-decoration:none; transition:color .18s; }
        .contact-link:hover { color:var(--li-text-main); }
        .contact-link-icon { font-size:16px; flex-shrink:0; }
        .contact-social-btn { display:flex; align-items:center; gap:8px; padding:10px 20px; border-radius:8px; border:1px solid var(--li-border); background:var(--li-bg); font-size:14px; font-weight:700; color:var(--li-text-muted); text-decoration:none; transition:all .18s ease; width:100%; }
        ...
        @media (max-width:768px) {
          ...
          .mobile-bottom-nav {
            ...
            z-index:1000; box-shadow:0 -4px 20px rgba(0,0,0,.05);
          }
          .mobile-nav-item { display:flex; flex-direction:column; align-items:center; gap:3px; color:var(--li-text-muted); text-decoration:none; font-size:10px; font-weight:700; transition:color .18s; }
