...
const Reveal = ({
  ...
  const hidden = { bottom: "translateY(36px)", left: "translateX(-40px)", right: "translateX(40px)" }[from];
  return (
  ...
  return (
    <section className="section" id="about">
        ...
        <Reveal from="left" delay={0.1} className="ab-left">
          <TiltCard className="ab-photo-wrap">
            ...
            {photo
              ...
              : <div className="ab-photo-placeholder"><span className="gradient-text">{name.charAt(0)}</span></div>
            }
            ...
            <div className="ab-details">
              {location && (
                <div className="ab-detail">
                  ...
                  <div><span className="ab-detail-lbl">Location</span><span className="ab-detail-val">{location}</span></div>
                </div>
              )}
              {email && (
                <div className="ab-detail">
                  <span className="ab-detail-ico">✉️</span>
                  <div><span className="ab-detail-lbl">Email</span><a href={`mailto:${email}`} className="ab-detail-val ab-detail-link">{email}</a></div>
                </div>
              )}
              {phone && (
                <div className="ab-detail">
                  <span className="ab-detail-ico">📞</span>
                  <div><span className="ab-detail-lbl">Phone</span><span className="ab-detail-val">{phone}</span></div>
                </div>
              )}
            </div>
          </Reveal>

          <Reveal delay={0.28}><p className="ab-section-label">WHAT I DO</p></Reveal>
      ...
      <style>{`
        ...
        .ab-grid { display:grid; grid-template-columns:360px 1fr; gap:60px; max-width:1100px; width:100%; align-items:start; }

        .ab-photo-wrap { position:relative; border-radius:22px; overflow:visible; cursor:default; }
        .ab-photo-frame { position:absolute; inset:-3px; border-radius:25px; background:linear-gradient(135deg,var(--li-purple),var(--li-cyan),var(--li-pink)); z-index:0; opacity:0.55; }
        .ab-photo { width:100%; display:block; border-radius:20px; position:relative; z-index:1; aspect-ratio:4/5; object-fit:cover; }
        .ab-photo-placeholder { width:100%; aspect-ratio:4/5; border-radius:20px; background:linear-gradient(135deg,rgba(62,24,249,.08),rgba(55,215,250,.08)); display:flex; align-items:center; justify-content:center; font-size:80px; font-weight:900; position:relative; z-index:1; }

        .ab-nameplate { position:absolute; bottom:0; left:0; right:0; z-index:3; background:rgba(255,255,255,.88); backdrop-filter:blur(14px); padding:14px 18px; border-radius:0 0 20px 20px; border-top:1px solid rgba(255,255,255,.5); }
        .ab-nameplate-name  { font-size:16px; font-weight:900; margin:0 0 3px; letter-spacing:-.02em; }
        .ab-nameplate-title { font-family:'IBM Plex Mono',monospace; font-size:11px; font-weight:700; color:var(--li-purple); margin:0; }

        .ab-exp-badge { position:absolute; top:-16px; right:-16px; z-index:5; background:white; border:1px solid var(--li-border); border-radius:14px; padding:12px 16px; text-align:center; box-shadow:0 10px 28px rgba(62,24,249,.14); animation:ab-float 4s ease-in-out infinite; }
        .ab-exp-num { display:block; font-size:26px; font-weight:900; letter-spacing:-.04em; line-height:1; }
        .ab-exp-lbl { display:block; font-family:'IBM Plex Mono',monospace; font-size:9px; font-weight:800; color:var(--li-text-muted); text-transform:uppercase; letter-spacing:.08em; }
        ...
        .ab-orbit-dot { width:10px; height:10px; border-radius:50%; background:var(--li-purple); box-shadow:0 0 12px var(--li-purple); }
        ...
        .ab-social-pill { padding:7px 14px; border-radius:6px; border:1px solid var(--li-border); background:white; font-size:12px; font-weight:800; color:var(--li-text-muted); text-decoration:none; font-family:'IBM Plex Mono',monospace; transition:all .18s ease; }
        .ab-social-pill:hover  { transform:translateY(-2px); box-shadow:0 6px 16px rgba(0,0,0,.08); }
        ...
        .ab-summary { font-size:14px; color:var(--li-text-muted); line-height:1.70; margin:0; padding:14px 16px; border-radius:10px; background:rgba(62,24,249,.04); border:1px solid rgba(62,24,249,.10); }
        ...
        .ab-detail-lbl  { display:block; font-family:'IBM Plex Mono',monospace; font-size:10px; font-weight:800; color:var(--li-text-muted); text-transform:uppercase; letter-spacing:.06em; margin-bottom:2px; }
        ...
        .ab-section-label { font-family:'IBM Plex Mono',monospace; font-size:10px; font-weight:800; color:var(--li-purple); letter-spacing:.12em; text-transform:uppercase; margin:0; padding-bottom:10px; border-bottom:1px solid rgba(62,24,249,.12); }

        .ab-feature-grid { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
        .ab-feature-card { background:white; border:1px solid var(--li-border); border-radius:14px; padding:20px 18px; cursor:default; position:relative; overflow:hidden; box-shadow:0 4px 14px rgba(0,0,0,.04); animation:ab-pop .5s cubic-bezier(.34,1.56,.64,1) both; }
        .ab-feature-card::before { content:""; position:absolute; top:0; left:0; right:0; height:2.5px; background:linear-gradient(90deg,var(--li-purple),var(--li-cyan)); opacity:0; transition:opacity .25s; }
        .ab-feature-card:hover::before { opacity:1; }
        .ab-feature-icon  { font-size:26px; margin-bottom:10px; }
        .ab-feature-title { font-size:14px; font-weight:800; margin:0 0 6px; letter-spacing:-.01em; }
        .ab-feature-desc  { font-size:12px; color:var(--li-text-muted); margin:0; line-height:1.55; }
        .ab-feature-shine { position:absolute; top:-40%; left:-40%; width:50%; height:180%; background:linear-gradient(90deg,transparent,rgba(255,255,255,.35),transparent); transform:skewX(-15deg); animation:ab-shimmer 3s ease-in-out infinite; }

        @media(max-width:1024px){ .ab-grid{grid-template-columns:1fr;gap:40px;} .ab-photo-wrap{max-width:340px;margin:0 auto;} .ab-orbit{display:none;} }
        @media(max-width:640px){ .ab-feature-grid{grid-template-columns:1fr;} }
      `}</style>
    </section>
  );
};
